...
export default function Products() {
    ...
    const increItem = (value) => {
        setProducts(value);
    }
    const decreItem = (value) => {
        setProducts(value);
    }
    return (
        <>
            <div className='main-container'>
                <div className='header-container'>
                    <div style={{ fontSize: '18px' }}>To spend <strong>{USD.format(totalMoney)}</strong> You have a lot of money</div>
                </div>
                <div className='product-container'>
                    <div className='grid-product'>
                        {products.map((e, i) => (
                            <div key={i}>
                                <Product data={e} list={products} increase={increItem} decrease={decreItem} />
                            </div>
                        ))}
                    </div>
                </div>
